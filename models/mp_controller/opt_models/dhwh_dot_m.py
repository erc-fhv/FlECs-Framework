...
class DHW_MILP_model_Temp(): 
    def __init__(self, name, P_nom=2_000, C_tes=4200*100, T_tes_min=12, T_tes_max=80, eta=1, T_out=50, T_in=12, c_p=4200):
        '''
    ...
    def pyo_block_rule(self, block):
        ...
        def dhwh_tes_constraint_energy_balance(block, p):
            return block.T_tes[p+1] == block.T_tes[p] + (block.P_el[p] * self.cop - (block.dot_m_demand[p] * self.c_p * (self.T_out - self.T_in))) * model.delta_t / self.C_tes
