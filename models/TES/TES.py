...
class TESModel():
    ...
    def step(self, dot_m_i_HP, dot_m_o_HP, dot_m_i_DHW, dot_m_o_DHW, T_i_HP, T_i_DHW, T_inf): #, time):
        ...
        self.x = logic(dot_m_i, dot_m_o, self.dot_m, self.UA, self.A, self.B, self.u, self.C, self.N, self.c_p, self.x, T_inf, T_i, self.delta_t)
        return {'T_0':self.x[0], 'T_1':self.x[1], 'T_2':self.x[2]}


def logic(dot_m_i, dot_m_o, dot_m, UA, A, B, u, C, N, c_p, x_prev, T_inf, T_i, delta_t):
    # check mass balance
